...
def compose_chat_messages(
    ...
    Args:
        ...
        assistant_messages (List[str], optional): List of messages
            from the assistant, corresponding to each user message.
        system_message (str, optional): A message from the system
            to set the behavior of the assistant.

    Returns:
        List[Dict[str, str]]: A list of message dictionaries
            suitable for the Chat Completions API.
    """
    messages = []
    if system_message:
